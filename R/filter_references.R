...
#' @description Function applying different kinds of filters on a list of references. The procedure applied is determined by the filter type.
...
filter_references <- function(
  ...
  if (base::is.null(filter_value[1]) | base::is.na(filter_value[1]) |
    ...
    if (filter_type == "logical") {
      ...
      dataset[stringr::str_detect(base::unlist(dataset[, variable]), rgx),]
      ...
      dataset <- dataset[dataset[, variable] >= filter_value[1],]
      dataset[dataset[, variable] <= filter_value[2],]
